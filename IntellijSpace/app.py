...
import gc
import time
import spaces  # for Hugging Face Spaces GPU
...
# -------------------------------------------------------------------------
@spaces.GPU(duration=120)
def api_predict_remote(input_images, text_prompt, interval=1, conf_thres=10, show_cam=False, apply_mask=False):
    """
    API endpoint for backend usage:
    - Accept uploaded images
    - Run reconstruction
    - Save predictions.npz and return file paths
    """
    if input_images is None or len(input_images) == 0:
        raise ValueError("No images provided")

    if text_prompt is None or len(str(text_prompt).strip()) == 0:
        raise ValueError("Text prompt is required")

    target_dir, _ = handle_uploads(None, input_images, interval=interval)
    if not target_dir or not os.path.isdir(target_dir):
        raise ValueError("Failed to create target directory from uploaded images")
    ...
    glb_path = os.path.join(target_dir, "api_result.glb")
    glbscene.export(file_obj=glb_path)

    return prediction_save_path, glb_path, target_dir
...
MODEL_REPO = "sosppxo/mvggt"
MODEL_FILE = "best_model/pytorch_model.bin"
...
try:
    ...
    print(f"Model downloaded from Hugging Face Hub: {ckpt_path}")
except Exception as e:
    print(f"Error downloading model from Hugging Face Hub: {e}")
    print("Please ensure the model is uploaded to https://huggingface.co/sosppxo/mvggt")
    raise
...
# Load tokenizer from Hugging Face (will auto-download)
...
with gr.Blocks(
    ...
    <div class="publication-header">
        ...
        <div class="publication-links">
            ...
            <span class="link-block">
                <a href="https://huggingface.co/sosppxo/mvggt" target="_blank">
                    <span class="icon" style="color: white;">🤗</span>
                    <span style="color: white;">Weights</span>
                </a>
            </span>
        </div>
    </div>
    ...
    scene0_images = sorted([os.path.abspath(f) for f in glob.glob("examples/scene0/*.jpg")])
    scene1_images = sorted([os.path.abspath(f) for f in glob.glob("examples/scene1/*.jpg")])
    scene2_video = os.path.abspath("examples/scene2/2.mp4") if os.path.exists("examples/scene2/2.mp4") else None
    scene3_video = os.path.abspath("examples/scene3/3.mp4") if os.path.exists("examples/scene3/3.mp4") else None
    scene4_video = os.path.abspath("examples/scene4/4.mp4") if os.path.exists("examples/scene4/4.mp4") else None
    
    examples = [
        [None, scene0_images, 1, 10, False, "the cabinet is in the northwest corner of the room. the cabinet is a white rectangular box.", True],
        [None, scene1_images, 1, 10, False, "there is a table in the corner between the couches with a vase on it.", True],
        [scene2_video, None, 30, 10, False, "It is a desk with a yellow tabletop. It has a metal frame (silver-colored) and a lower yellow storage shelf.", True],
        [scene3_video, None, 30, 10, False, "It is a bed with a black tufted headboard. The bedspread is white with black decorative patterns. There are items (such as dolls) placed on the bed.", True],
        [scene4_video, None, 30, 10, False, "It is a 3-seater sofa. Its upholstery is light beige. It has seat and back cushions, and black metal legs.", True],
    ]
    
    # Filter out examples with missing files
    examples = [ex for ex in examples if (ex[0] is not None and os.path.exists(ex[0])) or (ex[1] is not None and len(ex[1]) > 0 and all(os.path.exists(f) for f in ex[1]))]
    ...
    def example_pipeline(
        ...
        glbfile, log_msg, dropdown = gradio_demo(
            target_dir, conf_thres, frame_filter, show_cam, text_prompt, apply_mask
        )
        
        return glbfile, log_msg, target_dir, dropdown, image_paths

    gr.Markdown("## 4. Examples")
    gr.Markdown("""
    **Click any row below to load an example scene with its text description:**
    
    - **Scene 0**: the cabinet is in the northwest corner of the room. the cabinet is a white rectangular box.
    - **Scene 1**: there is a table in the corner between the couches with a vase on it.
    - **Scene 2**: It is a desk with a yellow tabletop. It has a metal frame (silver-colored) and a lower yellow storage shelf.
    - **Scene 3**: It is a bed with a black tufted headboard. The bedspread is white with black decorative patterns. There are items (such as dolls) placed on the bed.
    - **Scene 4**: It is a 3-seater sofa. Its upholstery is light beige. It has seat and back cushions, and black metal legs.
    ...
    # -------------------------------------------------------------------------
    # Backend-facing API for remote inference from external server
    # -------------------------------------------------------------------------
    api_images = gr.File(file_count="multiple", file_types=["image"], visible=False)
    api_text_prompt = gr.Textbox(visible=False)
    api_interval = gr.Number(value=1, visible=False)
    api_conf_thres = gr.Number(value=10, visible=False)
    api_show_cam = gr.Checkbox(value=False, visible=False)
    api_apply_mask = gr.Checkbox(value=False, visible=False)
    api_predictions_file = gr.File(visible=False)
    api_glb_file = gr.File(visible=False)
    api_target_dir = gr.Textbox(visible=False)
    api_btn = gr.Button("api_predict_remote", visible=False)

    api_btn.click(
        fn=api_predict_remote,
        inputs=[api_images, api_text_prompt, api_interval, api_conf_thres, api_show_cam, api_apply_mask],
        outputs=[api_predictions_file, api_glb_file, api_target_dir],
        api_name="predict_remote",
    )
# Get absolute path to examples directory for allowed_paths
examples_dir = os.path.abspath("examples")
allowed_paths = []
if os.path.exists(examples_dir):
    allowed_paths.append(examples_dir)
    # Also add all subdirectories
    for subdir in os.listdir(examples_dir):
        subdir_path = os.path.join(examples_dir, subdir)
        if os.path.isdir(subdir_path):
            allowed_paths.append(subdir_path)
