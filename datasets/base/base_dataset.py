...
from mvggt.utils.geometry import depthmap_to_absolute_camera_coordinates
import numpy as np
import os
import PIL
...
class BaseDataset(EasyDataset):
    ...
    def _crop_resize_if_necessary(self, image, depthmap, intrinsics, resolution, rng=None, info=None, normal=None, far_mask=None, instance_map=None):
        ...
        if self.mode == 'train':
            ...
            crop_bbox = (l, t, r, b)
            image, depthmap, intrinsics, normal, far_mask, instance_map = cropping.crop_image_depthmap(image, depthmap, intrinsics, crop_bbox, normal=normal, instance_map=instance_map)
        ...
        if self.aug_focal:
            ...
            image, depthmap, intrinsics, normal, far_mask, instance_map = cropping.center_crop_image_depthmap(image, depthmap, intrinsics, crop_scale, normal=normal, far_mask=far_mask, instance_map=instance_map)

        if self.aug_crop > 1:
            target_resolution += rng.integers(0, self.aug_crop)
        
        image, depthmap, intrinsics, normal, far_mask, instance_map = cropping.rescale_image_depthmap(image, depthmap, intrinsics, target_resolution, normal=normal, far_mask=far_mask, instance_map=instance_map)

        intrinsics2 = cropping.camera_matrix_of_crop(intrinsics, image.size, resolution, offset_factor=0.5)
        crop_bbox = cropping.bbox_from_intrinsics_in_out(intrinsics, intrinsics2, resolution)
        image, depthmap, intrinsics2, normal, far_mask, instance_map = cropping.crop_image_depthmap(image, depthmap, intrinsics, crop_bbox, normal=normal, far_mask=far_mask, instance_map=instance_map)

        other = [x for x in [normal, far_mask, instance_map] if x is not None]
        ...
        for _ in range(10):
            try:
                ...
                for v, view in enumerate(views):
                    ...
                    if 'instance_map' not in view:
                        view['instance_map'] = None

                for view in views:
                    view['img'] = self.transform(view['img'])
                    if 'instance_map' in view and view['instance_map'] is not None:
                        instance_map_np = np.array(view['instance_map'], dtype=np.int64)
                        view['instance_map'] = torch.from_numpy(instance_map_np)
